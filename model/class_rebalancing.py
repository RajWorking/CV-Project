...
from config import IMAGENET_IMAGES_PATH, IMG_ROWS, IMG_COLS, INTERPOLATION_METHOD, NUM_OF_NEIGHBOURS_REBALANCING, SMOOTHING_SIGMA, REBALANCING_GAMMA
...
def load_data(sample_size=10000, image_rows=IMG_ROWS, image_cols=IMG_COLS):
    ...
    for i, name in enumerate(subsampled_names):
        ...
        if INTERPOLATION_METHOD == 'INTER_AREA':
            bgr = cv2.resize(bgr, (image_rows, image_cols), interpolation = cv2.INTER_AREA)
        else:
            bgr = cv2.resize(bgr, (image_rows, image_cols), interpolation = cv2.INTER_CUBIC)
